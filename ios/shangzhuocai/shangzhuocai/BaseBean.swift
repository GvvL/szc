...
class BaseBean : NSObject {
	required override init() { }

	class func jsonToModel<T>(dics: AnyObject?) -> T! {
		if dics == nil {
			return nil
		}
		var dic: AnyObject!
			...
			if let b = AnyBidirectionalCollection(properties.children) {
				for i in b.startIndex ..< b.endIndex { // 因为是继承NSObject对象的，0是NSObject，所以从1开始
					...
					case is Float, is Double, is Bool, is NSNumber: // base type
						let value: AnyObject! = dic?.objectForKey(key!)?.floatValue
						if value != nil {
							obj.setValue(value, forKey: key!)
						}
						break
						...
						default:
							...
							if clz != nil {
								if let data = dic.objectForKey(key!) as? NSArray {
									let value = clz.json2ModelList(data)
									obj.setValue(value, forKey: key!)
								}
								else {
									let value = dic.objectForKey(key!)
									obj.setValue((clz as! BaseBean.Type).jsonToModel(value), forKey: key!)
								}
							}
							else {
	...
	class func json2ModelList(data: AnyObject?) -> Array<AnyObject> {
		if data == nil {
			return []
		}
